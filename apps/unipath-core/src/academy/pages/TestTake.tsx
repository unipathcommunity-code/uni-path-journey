...
const TestTake = () => {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const { data: t, error: te } = await supabase.from("tests").select("*").eq("id", testId).maybeSingle();
      ...
      // Existing attempt?
      const { data: existing } = await supabase.from("test_attempts")
      ...
      const { data: qs } = await supabase.from("test_questions")
      ...
      if (!aId) {
        ...
        const { data: created, error: ae } = await supabase.from("test_attempts").insert({
          test_id: testId,
          student_id: user.id,
  ...
  const submit = async (auto = false) => {
    ...
    try {
      ...
      const { data: keys } = await supabase.from("test_questions")
