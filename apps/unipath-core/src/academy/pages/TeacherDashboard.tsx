...
const TeacherDashboard = () => {
  ...
  const { data: lessons = [] } = useQuery({
    ...
    queryFn: async () => {
      if (!user) return [];
      let q = supabase.from("lessons").select("*, subjects(name), rooms(name)")
  ...
  const { data: students = [] } = useQuery({
    ...
    queryFn: async () => {
      if (!user) return [];
      let grpsQ = supabase.from("groups").select("id").eq("teacher_id", user.id);
      ...
      let tlQ = supabase.from("lessons").select("id").eq("teacher_id", user.id);
      ...
      const { data: hwIds } = await supabase.from("homework").select("id").eq("teacher_id", user.id);
