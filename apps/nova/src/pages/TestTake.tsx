...
const TestTake = () => {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const { data: t, error: te } = await sb.from("tests").select("*").eq("id", testId).maybeSingle();
      ...
      // Existing attempt?
      const { data: existing } = await sb.from("test_attempts")
        .select("*").eq("test_id", testId).eq("student_id", user.id).maybeSingle();
      if (existing?.submitted_at) {
        setDone({ score: existing.score, total: existing.total_points });
        setLoading(false);
        return;
      }

      const { data: qs } = await sb.from("test_questions")
      ...
      if (!aId) {
        ...
        const { data: created, error: ae } = await sb.from("test_attempts").insert({
          test_id: testId,
          student_id: user.id,
  ...
  const submit = async (auto = false) => {
    ...
    try {
      ...
      const { data: keys } = await sb.from("test_questions")
      ...
      const { error } = await sb.from("test_attempts").update({
